...
/**
 *
 * @param inputImageRGBA Input
 * @param numRows Input
 * @param numCols Input
 * @param redChannel Output channel
 * @param greenChannel Output channel
 * @param blueChannel Output channel
 */
...
__global__ void separateChannels(const uchar4 *const inputImageRGBA,
  ...
  int const xCoordinate = blockIdx.x * blockDim.x + threadIdx.x;
  int const yCoordinate = blockIdx.y * blockDim.y + threadIdx.y;
  int const pixelIndex = numCols * yCoordinate + xCoordinate;
...
void your_gaussian_blur(const uchar4 *const h_inputImageRGBA,
  ...
  int const blockDimension = 32; // In order to create square blocks with 1024 threads total
  dim3 const blockSize(blockDimension, blockDimension, 1);

  // Calculate blockGrid dimensions
  // Add 1 in order to round up
  int const blockGridXDim = numCols / blockDimension + 1;
  int const blockGridYDim = numRows / blockDimension + 1;

  dim3 const gridSize(blockGridXDim, blockGridYDim, 1);
